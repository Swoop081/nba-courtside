/* NBA Starting5 v0.13.0-dev.3 — Season scoreboard/result adapter. No gameplay wrappers or polling. */
(()=>{
  if(window.__starting5SeasonCoreUiV01303)return;
  window.__starting5SeasonCoreUiV01303=true;
  ...
  function paintScoreboard(){
    const ids=activeLineups(),game=document.getElementById('game');if(!ids||!game?.classList.contains('active'))return;const sides=[...game.querySelectorAll('.score-side')];if(sides.length<2)return;let us=0,cs=0;try{us=Number(state?.userScore)||0;cs=Number(state?.cpuScore)||0}catch{}
