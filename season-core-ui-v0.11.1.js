/* NBA Starting5 v0.13.0-dev.2 — Season scoreboard/result adapter. No gameplay wrappers or polling. */
(()=>{
  if(window.__starting5SeasonCoreUiV01302)return;
  window.__starting5SeasonCoreUiV01302=true;
  ...
  function rememberSeasonGame(){
    const s=read();if(!s)return;const round=s.schedule?.[s.roundIndex]||[],g=round.find(x=>String(x.home)===String(s.teamId)||String(x.away)===String(s.teamId));if(!g)return;
    const opp=String(g.home)===String(s.teamId)?String(g.away):String(g.home);try{sessionStorage.setItem(PENDING_KEY,JSON.stringify({roundIndex:s.roundIndex,gameId:g.id,userId:String(s.teamId),oppId:opp,home:String(g.home),away:String(g.away)}))}catch{}
  }
  function commitSeasonResult(scores){
    ...
    apply(g,winner,true,`${us}-${cs}`);for(const other of round){if(other.id===g.id||s.results[other.id])continue;apply(other,Math.random()<.5?String(other.home):String(other.away),false)}s.roundIndex=Math.max(Number(s.roundIndex)||0,p.roundIndex+1);if(s.roundIndex>=82)s.complete=true;write(s);return true;
  }
  function paintScoreboard(){
    const ids=activeLineups(),game=document.getElementById('game');if(!ids||!game?.classList.contains('active'))return;const sides=[...game.querySelectorAll('.score-side')];if(sides.length<2)return;
