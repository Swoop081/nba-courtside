/* NBA Starting5 v0.11.24 — final screen + Player of the Week loader + restart Season */
(()=>{
  if(window.__starting5FinalUiV01124)return;
  window.__starting5FinalUiV01124=true;
  ...
  function ensureRestartSeason(){
    ...
    actions.className='s5-season-head-actions';
    const restart=document.createElement('button');
    restart.id='s5RestartSeasonBtn';
    restart.className='ghost-btn s5-restart-season';
    restart.type='button';
    restart.textContent='New Season';
  ...
  style.textContent=`
    ...
    @media(max-width:430px){
      ...
      #seasonHub .s5-season-head-actions{gap:5px}
      #seasonHub .s5-season-head-actions .ghost-btn{padding-left:9px!important;padding-right:9px!important;font-size:10px!important}
    }
  ...
  document.addEventListener('click',e=>{
    if(e.target.closest('#seasonModeBtn'))setTimeout(ensureRestartSeason,0);
    const final=document.getElementById('final');
    if(!final?.classList.contains('active'))return;
  ...
  const start=()=>{ensureRestartSeason();enhanceFinal()};
