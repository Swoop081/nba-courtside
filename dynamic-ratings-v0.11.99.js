/* NBA Starting5 v0.11.99 — season-only dynamic matchup-streak ratings. */
(()=>{
  if(window.__starting5DynamicRatingsV01199)return;
  window.__starting5DynamicRatingsV01199=true;
  ...
  const installCardMarkup=()=>{
    ...
    if(typeof fn!=='function'||fn.__s5DynamicRatingsV01199Wrapped)return false;
    ...
    wrapped.__s5DynamicRatingsV01199Wrapped=true;window.cardMarkup=wrapped;try{eval('cardMarkup=window.cardMarkup')}catch{}return true;
  };
  const installPlayQuarter=()=>{
    ...
    const wrapped=function(){
      ...
      const season=isSeasonGameplay();
      const out=fn.apply(this,arguments);
      if(!season)return out;
      try{const hist=Array.isArray(state?.history)?state.history:[];if(hist.length>before){const h=hist[hist.length-1],u=Number(h?.userPts),c=Number(h?.cpuPts);if(Number.isFinite(u)&&Number.isFinite(c)&&u!==c)recordResult(u>c?h.user:h.cpu,u>c?h.cpu:h.user)}}catch(err){console.error('[Starting5 dynamic ratings]',err)}
      return out;
    };
    wrapped.__s5DynamicRatingsV01199Wrapped=true;window.playQuarter=wrapped;try{eval('playQuarter=window.playQuarter')}catch{}return true;
  };
