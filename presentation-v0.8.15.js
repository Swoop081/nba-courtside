/* NBA Courtside v0.8.43 — scoreboard recovery + reliable transparent logo pass */
(() => {
  /* The historical SSRef image endpoint introduced opaque/missing logos. Keep gameplay stable
     by using the NBA transparent SVG endpoint everywhere until historical logos are local assets. */
...
window.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{
  ...
  /* Clone Play to remove the original app.js listener which bypassed the presentation reset wrapper. */
...
window.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{
  ...
  const version=document.querySelector('.brand-version');if(version){version.textContent='v0.8.43';version.dataset.buildVersion='0.8.43';}
},180));
