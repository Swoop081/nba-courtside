...
(() => {
  ...
  beginQuarter=function(){originalBeginQuarter();ensureScoreboardTeams();};
...
window.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{
  ...
  const style=document.createElement('style');style.id='courtside-catalogue-set-mode-v0842';style.textContent=`
    ...
    .catalogue-team-browser.set-mode{grid-template-columns:1fr;padding:14px 8px 16px}.catalogue-team-browser.set-mode .catalogue-arrow,.catalogue-team-browser.set-mode .catalogue-team-logo{display:none!important}.catalogue-team-browser.set-mode .catalogue-set-logo{display:grid}.catalogue-grid.set-mode{grid-template-columns:repeat(3,minmax(0,1fr));gap:7px}`;document.head.appendChild(style);
  function setBadge(){if(!badge)return;if(activeSet==='Thunder & Lightning'){badge.className='catalogue-set-logo thunder-set-logo';badge.innerHTML='<span>NBA</span><strong>THUNDER</strong><b>& LIGHTNING</b>';}else{badge.className='catalogue-set-logo';badge.innerHTML='<span>NBA</span><strong>TIP-OFF</strong><b>27</b>';}}
  function renderFilters(){filter.innerHTML=['ALL',...sets].map(s=>`<button class="catalogue-set-btn ${activeSet===s?'active':''}" data-set="${s}">${s==='ALL'?'All Sets':s}</button>`).join('');filter.querySelectorAll('[data-set]').forEach(b=>b.onclick=()=>{activeSet=b.dataset.set;renderFilters();render();});}
  function render(){
    if(activeSet!=='ALL'){
      const cards=players.filter(p=>p.set===activeSet);browser.classList.add('set-mode');grid.classList.add('set-mode');if(logo)logo.style.display='none';setBadge();name.textContent=activeSet;meta.textContent=`${cards.length} CARDS · COMPLETE SET`;grid.innerHTML=cards.map(p=>cardMarkup(p,{})).join('');
    }else{
      browser.classList.remove('set-mode');grid.classList.remove('set-mode');if(logo)logo.style.display='block';const team=teams[teamIndex],cards=players.filter(p=>p.teamId===team.id);logo.src=`https://cdn.nba.com/logos/nba/${team.id}/global/L/logo.svg`;logo.alt=team.name;name.textContent=team.name;meta.textContent=`${cards.length} ${cards.length===1?'CARD':'CARDS'} · ALL SETS`;grid.innerHTML=Array.from({length:Math.max(9,cards.length)},(_,i)=>cards[i]?cardMarkup(cards[i],{}):`<div class="catalogue-empty"><img src="https://cdn.nba.com/logos/nba/${team.id}/global/L/logo.svg" alt=""></div>`).join('');
    }
    window.scrollTo({top:0});
  }
  prev.onclick=()=>{if(activeSet!=='ALL')return;teamIndex=(teamIndex-1+teams.length)%teams.length;render();};
  next.onclick=()=>{if(activeSet!=='ALL')return;teamIndex=(teamIndex+1)%teams.length;render();};
...
window.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{
  const intro=document.getElementById('intro'),actions=intro?.querySelector('.brand-launch-actions'),play=document.getElementById('startBtn'),catalogue=document.getElementById('catalogueBtn'),topOptions=document.getElementById('optionsBtn'),topNewGame=document.getElementById('newGameBtn');
...
/* One visible update control, and this file is the final authority for the displayed build. */
window.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{
