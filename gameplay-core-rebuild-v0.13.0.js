/* NBA Starting5 v0.13.0-dev.4 — single authoritative gameplay core rebuild.
   Owns matchup categories, card interaction, opponent rail, effective stats, CPU choice,
   scoring, history, round flow, overtime and final handoff. No polling and no gameplay wrappers. */
(()=>{
  if(window.__starting5GameplayCoreV01304)return;
  window.__starting5GameplayCoreV01304=true;
  ...
  const requiredPosition=()=>{try{return Array.isArray(state?.positionOrder)?state.positionOrder[Math.max(0,(Number(state.quarter)||1)-1)]||null:null}catch{return null}};
  const maxRounds=()=>Math.max(1,Number(state?.maxRounds)||4);
  ...
  function ratingIndicator(d){if(!d)return '';const n=Math.min(3,Math.abs(Math.trunc(d))),dir=d>0?'up':'down';return `<span class="s5-dynamic-rating s5-dynamic-rating-${dir}" aria-label="${d>0?'up':'down'} ${n}">${[1,2,3].map(i=>`<i class="${i<=n?'active':''}"></i>`).join('')}</span>`}
  function paintCardValues(card,p,played,side){
    ...
    card.querySelector(':scope>.s5-dynamic-value')?.remove();card.querySelector(':scope>.s5-dynamic-rating')?.remove();const d=delta(p),html=ratingIndicator(d);if(html)card.insertAdjacentHTML('beforeend',html);
  }
  function makeCard(p,side,eager=false){const played=playedFor(p,side),used=!!played||isUsed(p,side),active=played?.category||state?.category||null,t=document.createElement('template');t.innerHTML=cardMarkup(p,{activeStat:active,used,eager}).trim();const card=t.content.firstElementChild;if(!card)return '';paintCardValues(card,p,played,side);if(used)card.classList.add('used');const req=requiredPosition();if(side==='user'&&!used&&req&&String(p.position)!==String(req))card.classList.add('s5-position-locked');return card.outerHTML}
  ...
  function pickCpu(){const k=state?.category,req=requiredPosition();let available=(Array.isArray(cpuTeam)?cpuTeam:[]).filter(p=>!isUsed(p,'cpu'));if(req)available=available.filter(p=>String(p.position)===String(req));if(!available.length)return null;return available.reduce((best,p)=>effective(p,k)>effective(best,k)?p:best,available[0])}
  ...
  function beginQuarter(){
    ...
    const detail={quarter:state.overtime?'OT':state.quarter,category:state.category,overtime:!!state.overtime,requiredPosition:req,userScore:state.userScore,cpuScore:state.cpuScore};emit('s5:category-changed',detail);emit('s5:matchup-start',detail);
  }
  function resolvePick(userPlayer,cpuPlayer){
    ...
    state.usedUser.add(userPlayer.id);state.usedCpu.add(cpuPlayer.id);state.history.push(entry);syncScore();emit('s5:matchup-resolved',{entry,userScore:state.userScore,cpuScore:state.cpuScore,overtime:!!state.overtime});
  ...
  function playQuarter(playerId){if(busy||!state)return;const u=(Array.isArray(userTeam)?userTeam:[]).find(p=>String(p.id)===String(playerId)),req=requiredPosition();if(!u||isUsed(u,'user')||(req&&String(u.position)!==String(req)))return;const c=pickCpu();if(!c)return;busy=true;pendingCpu=c;const selected=document.querySelector(`#lineup .player-card[data-id="${CSS.escape(String(u.id))}"]`);selected?.classList.add('s5-selected-card');renderCpuStage('choosing');transitionTimer=setTimeout(()=>resolvePick(u,c),280)}
  function advance(){if(!state)return;if(state.overtime){finishGame();return}if(state.quarter>=maxRounds()){if(state.userScore===state.cpuScore&&!state.noOvertime){startOvertime();return}finishGame();return}state.quarter++;beginQuarter();window.scrollTo({top:0,behavior:'instant'})}
  function nextQuarter(){advance()}
  function startOvertime(){state.overtime=true;state.quarter=maxRounds()+1;emit('s5:overtime-start',{userScore:state.userScore,cpuScore:state.cpuScore});beginQuarter();window.scrollTo({top:0,behavior:'instant'})}
