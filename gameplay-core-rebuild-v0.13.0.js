...
(()=>{
  ...
  function paintCardValues(card,p,played,side){
    ...
    card.querySelector(':scope>.s5-dynamic-value')?.remove();const d=delta(p);if(d){const b=document.createElement('span');b.className='s5-dynamic-value';b.textContent=(d>0?'+':'')+d;card.appendChild(b)}
  }
  ...
  function syncScore(){let u=0,c=0;hist().forEach(h=>{u+=Number(h.userPts)||0;c+=Number(h.cpuPts)||0});state.userScore=u;state.cpuScore=c;const ue=document.getElementById('userScore'),ce=document.getElementById('cpuScore');if(ue)ue.textContent=String(u);if(ce)ce.textContent=String(c)}
  ...
  function resolvePick(userPlayer,cpuPlayer){
    const k=state.category,uv=effective(userPlayer,k),cv=effective(cpuPlayer,k);state.usedUser.add(userPlayer.id);state.usedCpu.add(cpuPlayer.id);state.history.push({quarter:state.overtime?'OT':state.quarter,category:k,user:userPlayer,cpu:cpuPlayer,userPts:uv,cpuPts:cv});syncScore();
  ...
  function finishGame(){clearTimeout(transitionTimer);busy=false;pendingCpu=null;showScreen('final');const fu=document.getElementById('finalUser'),fc=document.getElementById('finalCpu'),fr=document.getElementById('finalResult'),qs=document.getElementById('quarterSummary');if(fu)fu.textContent=String(state.userScore);if(fc)fc.textContent=String(state.cpuScore);if(fr)fr.textContent=state.userScore===state.cpuScore?'Game Tied':state.userScore>state.cpuScore?'Starting5 Victory':'Defeat';if(qs)qs.innerHTML=hist().map(h=>`<div class="summary-row"><strong>${h.quarter==='OT'?'OT':'Q'+h.quarter}</strong><span>${LABELS[h.category]} · ${h.user.name} vs ${h.cpu.name}</span><b>${h.userPts}–${h.cpuPts}</b></div>`).join('');window.scrollTo({top:0,behavior:'instant'})}
  function resetGame(){clearTimeout(transitionTimer);busy=false;pendingCpu=null;dealTeams();state={quarter:1,userScore:0,cpuScore:0,usedUser:new Set(),usedCpu:new Set(),category:null,history:[],overtime:false};showScreen('game');beginQuarter()}
