/* NBA Starting5 Season Mode v1.0.0 — clean 30-team, 82-game foundation */
(()=>{
  ...
  const teamPlayers=id=>{
    let pool=[];try{pool=Array.isArray(players)?players:[]}catch{}
    return pool.filter(p=>String(p.teamId)===String(id)&&!p.classicTeam).sort((a,b)=>(POS[a.position]??9)-(POS[b.position]??9)).slice(0,5);
  };
  ...
  function roundRobin(){
    ...
    for(let r=0;r<29;r++){
      const games=[];
      for(let i=0;i<15;i++){
        const x=a[i],y=a[29-i];
        const home=(r+i)%2===0?x:y,away=home===x?y:x;
  ...
  function buildSchedule(){
    ...
    for(let i=0;i<41;i++){
      const src=base[i%29];
      first41.push(src.map(g=>i<29?{...g}:{home:g.away,away:g.home}));
    }
    const out=[];
    first41.forEach((round,i)=>out.push(round.map(g=>({...g,round:i+1,id:`R${i+1}-${g.away}-${g.home}`}))));
    first41.forEach((round,i)=>out.push(round.map(g=>({home:g.away,away:g.home,round:i+42,id:`R${i+42}-${g.home}-${g.away}`}))));
    return out;
  }
  const emptyRecords=()=>Object.fromEntries(IDS.map(id=>[id,{w:0,l:0}]));
  const createSeason=teamId=>({version:1,teamId:String(teamId),roundIndex:0,schedule:buildSchedule(),records:emptyRecords(),results:{},complete:false,createdAt:new Date().toISOString()});
  ...
  function applyResult(s,g,winner,userPlayed=false,score=''){
    if(s.results[g.id])return;
    const loser=winner===g.home?g.away:g.home;
    s.results[g.id]={winner,loser,userPlayed,score};
    s.records[winner].w++;s.records[loser].l++;
  }
  function simulateRestOfRound(s,round,userGameId){
    for(const g of round){if(g.id===userGameId)continue;applyResult(s,g,simWinner(g.home,g.away),false);}
  }
  ...
  function ensureUI(){
    const intro=document.getElementById('intro');const actions=intro?.querySelector('.brand-launch-actions');
    ...
    pick.querySelector('[data-season-back]').addEventListener('click',()=>showScreen('intro'));
    hub.querySelector('[data-season-home]').addEventListener('click',()=>showScreen('intro'));
  }
  function renderTeamPicker(){
    ...
    grid.querySelectorAll('[data-team]').forEach(b=>b.addEventListener('click',()=>{const s=createSeason(b.dataset.team);write(s);renderHub();showScreen('seasonHub');window.scrollTo(0,0);}));
  }
  ...
  function renderHub(){
    ...
    c.innerHTML=hero+next+`<div class="s5-standings">${tableMarkup('Eastern Conference',east,s)}${tableMarkup('Western Conference',west,s)}</div>`;
    document.getElementById('s5PlaySeasonGame')?.addEventListener('click',startGame);
  }
  function startGame(){
    ...
    activeGame={roundIndex:s.roundIndex,gameId:g.id,userId:s.teamId,oppId:opp,home:g.home,away:g.away};
    userTeam=ut;cpuTeam=ct;state={quarter:1,userScore:0,cpuScore:0,usedUser:new Set(),usedCpu:new Set(),category:null,history:[],overtime:false};showScreen('game');beginQuarter();window.scrollTo(0,0);
  ...
  if(typeof baseFinish==='function'){
    finishGame=function(){
      ...
      const snapshot={...activeGame},s=read();
      if(!s){activeGame=null;return baseFinish.apply(this,arguments);}
      const round=s.schedule[s.roundIndex]||[],g=round.find(x=>x.id===snapshot.gameId);
      let userWon=state.userScore>state.cpuScore;if(state.userScore===state.cpuScore)userWon=Math.random()>=.5;
      applyResult(s,g,userWon?s.teamId:snapshot.oppId,true,`${state.userScore}-${state.cpuScore}`);
      simulateRestOfRound(s,round,g.id);s.roundIndex++;if(s.roundIndex>=82)s.complete=true;write(s);
      const out=baseFinish.apply(this,arguments);
      const rec=s.records[s.teamId];document.getElementById('finalResult').textContent=userWon?`${short(s.teamId)} Win!`:`${short(s.teamId)} Lose`;
      const btn=document.getElementById('playAgainBtn');if(btn)btn.textContent=s.complete?'View Final Standings':`Continue · ${rec.w}-${rec.l}`;
      activeGame={...snapshot,finished:true};return out;
    };
  }

  document.addEventListener('click',e=>{
    const btn=e.target.closest('#playAgainBtn,#compactPlayAgain');if(!btn||!activeGame?.finished)return;
    e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();activeGame=null;renderHub();showScreen('seasonHub');window.scrollTo(0,0);
  },true);

  function start(){ensureUI();renderTeamPicker();const b=document.getElementById('seasonModeBtn');if(b&&read())b.textContent='Continue Season';}
