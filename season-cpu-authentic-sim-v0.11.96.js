/* NBA Starting5 v0.13.0-dev.4 — event-driven CPU season simulation and dynamic-rating replay. */
(()=>{
  if(window.__starting5CpuAuthenticSimV01304)return;
  window.__starting5CpuAuthenticSimV01304=true;
  ...
  function simulateCpuGame(g){
    ...
    const r=rng(hash(`v01304|${g.id}`)),hu=new Set(),au=new Set(),out=[];let hs=0,as=0;
    ...
    for(let i=0;i<5;i++)play(false);let guard=0;while(hs===as&&guard++<5){hu.clear();au.clear();play(true)}return out;
  }
  ...
  function processIncremental(s){
    ...
    for(const round of s.schedule){for(const g of round||[]){const res=s.results?.[g.id];if(!res||res.userPlayed||res.cpuSimulationVersion===3)continue;res.matchups=simulateCpuGame(g);res.matchupLedgerSource='cpu-authentic-dynamic-v3';res.cpuSimulationVersion=3;res.dynamicRatingsApplied=true;simCount++;changed=true}}
    if(changed){s.matchupLedgerVersion=3;s.cpuAuthenticSimulationVersion=3;s.cpuAuthenticSimulation={incremental:true,gamesProcessed:simCount,updatedAt:Date.now()};refreshAwards(s)}
    return changed;
  }
  function rebuildFromSave(s){
    ...
    for(const round of s.schedule){for(const g of round||[]){const res=s.results?.[g.id];if(!res)continue;if(res.userPlayed){replayStoredMatchups(res);res.dynamicRatingsApplied=true;matchupCount+=(res.matchups||[]).length;continue}res.matchups=simulateCpuGame(g);res.matchupLedgerSource='cpu-authentic-dynamic-v3';res.cpuSimulationVersion=3;res.dynamicRatingsApplied=true;simCount++;matchupCount+=res.matchups.length;changed=true}}
    s.matchupLedgerVersion=3;s.cpuAuthenticSimulationVersion=3;s.cpuAuthenticSimulation={games:simCount,matchups:matchupCount,rebuiltAt:Date.now()};refreshAwards(s);return changed;
  }
