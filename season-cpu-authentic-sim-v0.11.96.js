/* NBA Starting5 v0.13.0-dev.6 — incremental CPU season simulation and dynamic-rating ledger. */
(()=>{
  if(window.__starting5CpuAuthenticSimV01306)return;
  window.__starting5CpuAuthenticSimV01306=true;
  const SAVE_KEY='nbaStarting5SeasonV2',CATS=['scoring','dunks','three','rebounding','passing','blocks','steals'];
  ...
  function simulateCpuGame(g){
    ...
    const r=rng(hash(`v01306|${g.id}`)),hu=new Set(),au=new Set(),out=[];let hs=0,as=0;
    ...
    for(let i=0;i<4;i++)play(false);if(hs===as)play(true);return out;
  }
  ...
  function processIncremental(s){
    ...
    if(changed){s.matchupLedgerVersion=4;s.cpuAuthenticSimulationVersion=4;s.cpuAuthenticSimulation={incremental:true,gamesProcessed:simCount,updatedAt:Date.now()};refreshAwards(s)}return changed;
  }
  ...
  const writeSeason=s=>{try{localStorage.setItem(SAVE_KEY,JSON.stringify(s))}catch{}};
  window.addEventListener('s5:season-result-committed',e=>{
    const s=e.detail?.season,p=e.detail?.pending;if(!s||!p)return;const res=s.results?.[p.gameId];if(!res)return;const actual=normalizeActual(e.detail?.history||[]);if(actual.length){res.matchups=actual;res.matchupLedgerSource='actual';res.dynamicRatingsApplied=true}processIncremental(s);refreshAwards(s);writeSeason(s);
  });
  const boot=()=>{try{const s=JSON.parse(localStorage.getItem(SAVE_KEY)||'null');if(s&&processIncremental(s))writeSeason(s)}catch(err){console.error('[Starting5 CPU sim boot]',err)}};
