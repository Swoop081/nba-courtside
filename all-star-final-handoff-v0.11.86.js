/* NBA Starting5 v0.13.0-dev.3 — All-Star final handoff adapter using canonical gameplay events. */
(()=>{
  if(window.__starting5AllStarFinalHandoffV01303)return;
  window.__starting5AllStarFinalHandoffV01303=true;
  ...
  const ensureSaved=detail=>{
    if(!active())return;const s=read();if(!s)return;s.allStarWeekend=s.allStarWeekend||{};
    if(!s.allStarWeekend.allStarGame?.complete){
      ...
      s.allStarWeekend.allStarGame={complete:true,winner:u===c?'TIE':u>c?userName:cpuName,score:`${u}-${c}`,mvp:mvp(history),history,completedAt:new Date().toISOString(),engine:'starting5-gameplay-core'};
    }
    const g=s.allStarWeekend.allStarGame;
    s.allStarWeekend.allStar={complete:true,winner:g.winner,score:g.score,mvp:g.mvp,history:g.history,completedAt:g.completedAt,engine:g.engine||'starting5-gameplay-core'};
    write(s);
  };
  ...
  const goSeason=()=>{const game=document.getElementById('game');game?.classList.remove('s5-all-star-standard-game','s5-all-star-game');window.STARTING5_SEASON_MODE?.renderHub?.();try{showScreen('seasonHub')}catch{document.querySelectorAll('.screen').forEach(x=>x.classList.remove('active'));document.getElementById('seasonHub')?.classList.add('active')}window.scrollTo(0,0)};
