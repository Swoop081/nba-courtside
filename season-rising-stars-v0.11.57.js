/* NBA Starting5 v0.13.0-dev.2 — Rising Stars roster/UI adapter for the canonical gameplay core. */
(()=>{
  if(window.__starting5RisingStarsV01302)return;
  window.__starting5RisingStarsV01302=true;
  ...
  function startGame(rosters){
    ...
    try{
      userTeam=user;cpuTeam=cpu;state={quarter:1,userScore:0,cpuScore:0,usedUser:new Set(),usedCpu:new Set(),category:null,history:[],overtime:false};
      const game=document.getElementById('game');game?.classList.add('s5-rising-stars-game');showScreen('game');
      window.dispatchEvent(new CustomEvent('s5:special-game-start',{detail:{type:'rising-stars',userName:userEast?'EAST':'WEST',cpuName:userEast?'WEST':'EAST'}}));
      beginQuarter();window.scrollTo(0,0);
    }catch(err){console.error('[Starting5 Rising Stars]',err);alert('Rising Stars could not start. Reload the game and try again.')}
  }

  function refreshHubButton(){
    const s=read(),btn=document.getElementById('s5PlaySeasonGame');if(!s||!btn)return;const p=played(s);if(p!==41||complete(s))return;
    if(btn.textContent!=='All-Star Weekend — Rising Stars')btn.textContent='All-Star Weekend — Rising Stars';const label=btn.closest('.s5-next')?.querySelector('.s5-next-label');if(label&&label.textContent!=='ALL-STAR WEEKEND · BETWEEN GAMES 41 & 42')label.textContent='ALL-STAR WEEKEND · BETWEEN GAMES 41 & 42';
  }
  ...
  window.addEventListener('s5:game-finished',()=>{if(document.getElementById('game')?.classList.contains('s5-rising-stars-game'))setTimeout(()=>document.getElementById('game')?.classList.remove('s5-rising-stars-game'),0)});
