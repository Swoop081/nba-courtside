...
raw=(ROOT/'data/data-v0.38.js').read_text().strip();prefix='window.NBA_COURTSIDE_DATA = ';assert raw.startswith(prefix)
...
idx=(ROOT/'index.html').read_text();assert 'V0.38 STAFF CAREERS' in idx;assert 'exhibition.html?v=0.38' in idx
assert 'index.html?continue=1&v=0.38' in (ROOT/'gameday-v0.38.js').read_text();assert 'index.html?v=0.38' in (ROOT/'exhibition-v0.38.js').read_text()
assert (ROOT/'app.js').read_bytes()==(ROOT/'app-v0.38.js').read_bytes()
assert (ROOT/'gameday.js').read_bytes()==(ROOT/'gameday-v0.38.js').read_bytes()
assert (ROOT/'exhibition.js').read_bytes()==(ROOT/'exhibition-v0.38.js').read_bytes()
print('PASS v0.38 cache coherence: release-unique NBA + Staff Careers + G League + College/Draft runtime assets')
