...
ck((ROOT/'gameday.js').read_bytes()==(ROOT/'gameday-v0.33.js').read_bytes(),'canonical Game Day matches v0.33 runtime')
ck((ROOT/'exhibition.js').read_bytes()==(ROOT/'exhibition-v0.33.js').read_bytes(),'canonical Exhibition matches v0.33 runtime')
...
if errors:
    print(f'FAIL — v0.33 retained ratings tests ({len(errors)} issues)')
    for e in errors[:80]: print(' -',e)
    sys.exit(1)
print('PASS — v0.33 retained ratings/source tests')
