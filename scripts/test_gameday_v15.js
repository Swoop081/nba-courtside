...
const store={'nbaCourtsideSaveV15':JSON.stringify(save),'nbaCourtsideTeam':'TOR'};
...
t.startGame(false);const rot=t.getRotations().home;const victim=rot.players[0];const pending=t.makePendingInjury(victim,'home');if(!pending)throw new Error('could not force game injury');t.simGame();const g=t.getGame(),sv=t.getSave();if(!g.final)throw new Error('not final');if(!sv.results.G0002)throw new Error('result missing');if(sv.results.G0002.engine!=='courtside_v15_possession')throw new Error('wrong engine '+sv.results.G0002.engine);if(!sv.injuries[victim.id])throw new Error('medical injury not persisted');if(!sv.results.G0002.injuries?.length)throw new Error('result injury missing');if(!sv.injuryHistory.length)throw new Error('injury history missing');
