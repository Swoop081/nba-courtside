...
const root='/mnt/data/NBA-Courtside-Data-Calibration-v0.16';
...
const save={version:12,userTeam:'TOR',seasonYear:2026,date:'2026-10-20',phase:'regular_season',seasonStarted:true,seasonComplete:false,assignments,statusOverrides:{},salaryOverrides:{},results:{},seasonStats:{},gameLogs:{},rotations:{},generatedPlayers:[],playerOverrides:{},injuries:{},injuryHistory:[],injuryRotationBackups:{},transactions:[],rng:20260819};
const store={'nbaCourtsideSaveV16':JSON.stringify(save),'nbaCourtsideTeam':'TOR'};
...
t.startGame(false);const rot=t.getRotations().home;const victim=rot.players[0];const pending=t.makePendingInjury(victim,'home');if(!pending)throw new Error('could not force game injury');t.simGame();const g=t.getGame(),sv=t.getSave();if(!g.final)throw new Error('not final');if(!sv.results.G0002)throw new Error('result missing');if(sv.results.G0002.engine!=='courtside_v16_possession')throw new Error('wrong engine '+sv.results.G0002.engine);if(!sv.injuries[victim.id])throw new Error('medical injury not persisted');if(!sv.results.G0002.injuries?.length)throw new Error('result injury missing');if(!sv.injuryHistory.length)throw new Error('injury history missing');
