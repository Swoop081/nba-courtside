...
const save={version:8,userTeam:'TOR',seasonYear:2026,date:'2026-10-20',phase:'regular_season',seasonStarted:true,seasonComplete:false,assignments,statusOverrides:{},salaryOverrides:{},results:{},seasonStats:{},gameLogs:{},rotations:{},generatedPlayers:[],playerOverrides:{},rng:20260819};
const store={'nbaCourtsideSaveV08':JSON.stringify(save),'nbaCourtsideTeam':'TOR'};
...
t.startGame(false);t.simGame();const g=t.getGame(),sv=t.getSave();if(!g.final)throw new Error('game not final');if(g.homeScore===g.awayScore)throw new Error('tie');if(!sv.results.G0002)throw new Error('result not committed');if(sv.results.G0002.engine!=='courtside_v08_possession')throw new Error('wrong engine');if(!sv.results.G0002.box?.home?.length||!sv.results.G0002.box?.away?.length)throw new Error('box missing');
