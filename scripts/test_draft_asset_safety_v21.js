...
vm.runInContext(src,ctx,{filename:'app-test-v21-assets.js',timeout:120000});
...
if(locked.length!==190)throw new Error('expected 190 non-severable/protected/frozen/source-locked cells, got '+locked.length);
...
const cle31pending=key('CLE',2031,1),sac32pending=key('SAC',2032,2);
if(cle31pending.owner!=='CLE'||cle31pending.tradeable!==false||cle31pending.sourceStatus!=='source_locked')throw new Error('CLE 2031 pending transfer must stay locked at CLE');
if(sac32pending.owner!=='CLE'||sac32pending.tradeable!==false||sac32pending.sourceStatus!=='source_locked')throw new Error('SAC 2032 pending onward transfer must stay locked at CLE');
...
// Conditional/protected/frozen/source-locked assets must never appear in the tradeable-only picker for their current owner.
...
if(ownerTradeable.some(x=>x.id===sampleLocked.id))throw new Error('nontradeable linked asset leaked into tradeable picker: '+sampleLocked.id);
...
console.log(JSON.stringify({
  ...
  statusCounts:(ctx.NBA_COURTSIDE_FUTURE_PICKS.cells||[]).reduce((m,x)=>(m[x.status]=(m[x.status]||0)+1,m),{}),
