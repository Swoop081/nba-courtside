...
const dataJs=fs.readFileSync(root+'/data/data-v0.42.js','utf8');
const schedJs=fs.readFileSync(root+'/data/schedule-v0.42.js','utf8');
const templateJs=fs.readFileSync(root+'/data/schedule-template-v0.42.js','utf8');
let src=fs.readFileSync(root+'/app-v0.42.js','utf8');
...
const ctx={console,structuredClone,Date,URLSearchParams,setTimeout:(f)=>f(),confirm:()=>true,scrollTo:noop,location:{reload:noop},window:{},document:{documentElement:{style:{setProperty:noop}},head:{appendChild:noop},querySelector:()=>null,querySelectorAll:()=>[],getElementById:()=>null,createElement:()=>({classList:{add:noop,remove:noop}}),body:{appendChild:noop,style:{}}},localStorage:{getItem:k=>store[k]||null,setItem:(k,v)=>store[k]=v,removeItem:k=>delete store[k]},Math};
ctx.window=ctx;vm.createContext(ctx);vm.runInContext(dataJs,ctx);vm.runInContext(fs.readFileSync(root+'/data/source-certification-v0.42.js','utf8'),ctx);vm.runInContext(fs.readFileSync(root+'/data/future-pick-ledger-v0.42.js','utf8'),ctx);vm.runInContext(schedJs,ctx);vm.runInContext(templateJs,ctx);vm.runInContext(fs.readFileSync(root+'/cba-v0.42.js','utf8'),ctx);vm.runInContext(fs.readFileSync(root+'/data/organizations-v0.42.js','utf8'),ctx);vm.runInContext(fs.readFileSync(root+'/data/staff-careers-v0.42.js','utf8'),ctx);vm.runInContext(fs.readFileSync(root+'/data/g-league-v0.42.js','utf8'),ctx);vm.runInContext(fs.readFileSync(root+'/data/college-draft-v0.42.js','utf8'),ctx);vm.runInContext(src,ctx);
