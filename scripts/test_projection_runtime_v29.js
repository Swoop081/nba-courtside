...
const proj=P.filter(p=>p.rating_source==='projection_translation_model_v0.29_rebased'), fin=P.filter(p=>p.stat_source_status==='season_complete_verified');
...
assert(SC.version==='v0.29'&&SC.projection_model.source_backed_players===49&&SC.projection_model.v029_display_rebase===true,'source certification v0.29');assert(by('yanic-niederhauser').stats_2025_26?.gp===41,'Yanic final NBA repair');
