...
for rel,want in manifest['files'].items(): ck(hashlib.sha256((ROOT/rel).read_bytes()).hexdigest()==want,f'v0.28 critical file unchanged {rel}')
...
if errors:
    print(f'FAIL — v0.29 ratings tests ({len(errors)} issues)')
    for e in errors[:80]: print(' -',e)
    sys.exit(1)
print('PASS — v0.29 ratings tests')
