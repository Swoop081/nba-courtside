...
ck((ROOT/'gameday.js').read_bytes()==(ROOT/'gameday-v0.36.js').read_bytes(),'canonical Game Day matches v0.36 runtime')
ck((ROOT/'exhibition.js').read_bytes()==(ROOT/'exhibition-v0.36.js').read_bytes(),'canonical Exhibition matches v0.36 runtime')
...
if errors:
    print(f'FAIL — v0.36 retained ratings tests ({len(errors)} issues)')
    for e in errors[:80]: print(' -',e)
    sys.exit(1)
print('PASS — v0.36 retained ratings/source tests')
