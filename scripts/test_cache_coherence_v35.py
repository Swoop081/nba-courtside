...
for page,refs in expected.items():
    ...
    assert not any('v0.34' in x for x in srcs),(page,srcs)
raw=(ROOT/'data/data-v0.35.js').read_text().strip();prefix='window.NBA_COURTSIDE_DATA = ';assert raw.startswith(prefix)
...
idx=(ROOT/'index.html').read_text();assert 'V0.35 COLLEGE + DRAFT WORLD' in idx;assert 'exhibition.html?v=0.35' in idx
assert 'index.html?continue=1&v=0.35' in (ROOT/'gameday-v0.35.js').read_text();assert 'index.html?v=0.35' in (ROOT/'exhibition-v0.35.js').read_text()
print('PASS v0.35 cache coherence: release-unique NBA + G League + College/Draft runtime assets')
