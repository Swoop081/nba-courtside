...
app=(ROOT/'app-v0.38.js').read_text(encoding='utf-8')
...
ck('V0.38 STAFF CAREERS' in idx,'menu v0.38 badge')
expected={
...
for page,refs in expected.items():
    ...
    ck(srcs==refs,f'{page} exact v0.38 runtime order')
...
if errors:
    print(f'FAIL — v0.38 League Events certification ({len(errors)} issues)')
    for e in errors: print(' -',e)
    sys.exit(1)
print('PASS — v0.38 League Events + Season Moments certification')
print(json.dumps({'release':'v0.38-retained-v37-events','save_schema':25,'event_state_version':37,'all_star_roster':24,'award_categories':6,'all_nba_teams':3,'all_defense_teams':2,'all_rookie_teams':2,'lottery_teams':16,'summer_league_games':5},indent=2))
