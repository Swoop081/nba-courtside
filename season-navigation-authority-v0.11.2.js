/* NBA Starting5 v0.11.2 — authoritative Season game navigation + scoreboard, no polling */
(()=>{
  if(window.__starting5SeasonNavigationAuthorityV0112)return;
  window.__starting5SeasonNavigationAuthorityV0112=true;
  ...
  function labelFinal(){
    ...
    const compact=document.getElementById('compactPlayAgain');
    const legacy=document.getElementById('playAgainBtn');
    if(compact)compact.textContent=label;
  ...
  function returnToSeason(){
    ...
    if(seasonBtn){seasonBtn.click();return;}
    if(typeof window.showScreen==='function')window.showScreen('intro');
  }

  // Window capture runs before document/target handlers. This makes Continue impossible
  // to fall through to the normal Play Again/resetGame route.
  window.addEventListener('click',e=>{
    const launch=e.target?.closest?.('#s5PlaySeasonGame');
    if(launch){
    ...
    const cont=e.target?.closest?.('#compactPlayAgain,#playAgainBtn');
  ...
  function start(){
    const final=document.getElementById('final');
    if(final)new MutationObserver(()=>{if(final.classList.contains('active'))requestAnimationFrame(labelFinal)}).observe(final,{attributes:true,attributeFilter:['class'],childList:true,subtree:false});
