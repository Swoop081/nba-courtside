/* NBA Courtside v0.8.67 — in-game Card Art Editor */
(() => {
  ...
  function installUI(){
    ...
    const style=document.createElement('style');style.id='card-art-editor-style-v0867';style.textContent=css;document.head.appendChild(style);
    ...
    const exportData=()=>{const store=read(),cards={};players.forEach(p=>{cards[p.artSlug]={...(store[p.artSlug]||baseFor(p)),edited:!!store[p.artSlug],name:p.name,set:p.set,team:p.teamShort};});return {format:'NBA Courtside Art Layout',version:1,gameVersion:'0.8.67',exportedAt:new Date().toISOString(),cards};};
