/* NBA Starting5 v0.11.91 — dynamic matchup-streak ratings with persistent hollow chevrons. */
(()=>{
  if(window.__starting5DynamicRatingsV01191)return;
  window.__starting5DynamicRatingsV01191=true;
  ...
  const row=p=>{
    const k=playerKey(p),v=streaks[k]||{};
  ...
  const chevronMarkup=p=>{
    ...
    const label=d===0?'No active rating streak':`${d>0?'+':''}${d} all stats`;
    return `<div class="s5-dynamic-rating s5-dynamic-rating-${dir} ${d===0?'s5-dynamic-rating-neutral':''}" aria-label="${label}">${items}</div>`;
  };

  const installCardMarkup=()=>{
    let fn=null;try{fn=window.cardMarkup||eval('cardMarkup')}catch{}
    if(typeof fn!=='function'||fn.__s5DynamicRatingsV01191Wrapped)return false;
    ...
    wrapped.__s5DynamicRatingsV01191Wrapped=true;
  ...
  css.id='s5-dynamic-ratings-style-v01191';
  css.textContent=`
    .player-card .s5-dynamic-rating{position:absolute;z-index:28;top:8px;right:8px;width:29px;display:flex;flex-direction:column-reverse;align-items:center;gap:1px;pointer-events:none;filter:drop-shadow(0 2px 2px rgba(0,0,0,.72))}
    .player-card .s5-dynamic-rating i{display:block;position:relative;width:27px;height:13px;clip-path:polygon(50% 0,100% 58%,78% 58%,50% 29%,22% 58%,0 58%,0 79%,22% 79%,50% 50%,78% 79%,100% 79%,100% 58%,50% 0);background:rgba(255,255,255,.26);filter:drop-shadow(0 0 1px rgba(255,255,255,.35))}
    .player-card .s5-dynamic-rating-up i.active{background:#28c856;filter:drop-shadow(0 0 4px rgba(40,200,86,.9)) drop-shadow(0 0 8px rgba(40,200,86,.45))}
    .player-card .s5-dynamic-rating-down{flex-direction:column}
    .player-card .s5-dynamic-rating-down i{transform:rotate(180deg)}
    .player-card .s5-dynamic-rating-down i.active{background:#ef3340;filter:drop-shadow(0 0 4px rgba(239,51,64,.9)) drop-shadow(0 0 8px rgba(239,51,64,.45))}
    .player-card .s5-dynamic-rating-neutral i{background:rgba(255,255,255,.24);filter:drop-shadow(0 0 1px rgba(255,255,255,.32))}
    @media(max-width:430px){.player-card .s5-dynamic-rating{top:7px;right:7px;width:26px}.player-card .s5-dynamic-rating i{width:24px;height:12px}}
  `;
  ...
  window.STARTING5_DYNAMIC_RATINGS={getDelta:p=>deltaFor(p),getStreak:p=>row(p),apply:applyPlayer,resetAll:()=>{streaks={};write(streaks);applyAll()}};
})();
