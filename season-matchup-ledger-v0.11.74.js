/* NBA Starting5 v0.13.0-dev.6 — pure matchup +/- ledger for awards and All-Star selection. */
(()=>{
  if(window.__starting5MatchupLedgerV01306)return;
  window.__starting5MatchupLedgerV01306=true;
  const CATS=['scoring','dunks','three','rebounding','passing','blocks','steals'],DEF=new Set(['blocks','steals']),POS=['PG','SG','SF','PF','C'];
  const pool=()=>{try{return Array.isArray(players)?players.filter(p=>!p.classicTeam):[]}catch{return []}};
  const key=p=>String(p?.id||p?.playerId||`${p?.teamId}|${p?.name}|${p?.position}`),stat=(p,c)=>Number(p?.stats?.[c]??0)||0;
  ...
  function simulateGame(g){const home=teamPlayers(g.home),away=teamPlayers(g.away);if(home.length<5||away.length<5)return[];const r=rng(hash(g.id)),hu=new Set(),au=new Set(),out=[];let hs=0,as=0;const play=()=>{const cat=CATS[Math.floor(r()*CATS.length)],h=bestUnused(home,hu,cat),a=bestUnused(away,au,cat);if(!h||!a)return;hu.add(key(h));au.add(key(a));const e=entry(cat,h,a);out.push(e);if(e.a.diff>0)hs++;else if(e.a.diff<0)as++};for(let i=0;i<4;i++)play();if(hs===as)play();return out}
  function enrich(s){if(!s?.results)return false;const gm=gameMap(s);let changed=false;for(const [id,res] of Object.entries(s.results)){if(Array.isArray(res.matchups)&&res.matchups.length)continue;const g=gm.get(id);if(!g)continue;res.matchups=simulateGame(g);res.matchupLedgerSource=res.userPlayed?'legacy-simulated':'simulated';changed=true}if(changed)s.matchupLedgerVersion=Math.max(Number(s.matchupLedgerVersion)||0,2);return changed}
