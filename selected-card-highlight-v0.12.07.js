/* NBA Starting5 v0.12.07 — selection highlight only after user tap; active CPU card also highlighted. */
(()=>{
  if(window.__starting5SelectedCardHighlightV01207)return;
  window.__starting5SelectedCardHighlightV01207=true;

  const style=document.createElement('style');
  style.id='s5-selected-card-highlight-v01207';
  style.textContent=`
  ...
  document.addEventListener('pointerdown',e=>{
    ...
    if(!card||card.classList.contains('used')||document.getElementById('lineup')?.classList.contains('result-open'))return;
    manualChoice=true;
    clearUser();
  ...
  const wrapBegin=()=>{
    ...
    if(typeof fn!=='function'||fn.__s5SelectionBeginV01207)return false;
    const wrapped=function(){manualChoice=false;clearAll();const out=fn.apply(this,arguments);setTimeout(clearAll,0);return out};
    wrapped.__s5SelectionBeginV01207=true;window.beginQuarter=wrapped;try{eval('beginQuarter=window.beginQuarter')}catch{}return true;
  };

  const wrapPlay=()=>{
    let fn=null;try{fn=window.playQuarter||eval('playQuarter')}catch{}
    if(typeof fn!=='function'||fn.__s5SelectionPlayV01207)return false;
    ...
    wrapped.__s5SelectionPlayV01207=true;window.playQuarter=wrapped;try{eval('playQuarter=window.playQuarter')}catch{}return true;
  };
